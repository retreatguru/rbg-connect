...
if (! empty($rs_the_programs)) {
    ...
    foreach($rs_the_programs as $program):
        ...
        <div class="rs-program rs-group <?php foreach($program->categories as $category) {echo $category->slug . " ";} ?>"
            ...
            <?php if (! empty($program->photo_details) && empty($hide_photo)) : ?>
                <?php $program_image_url = ! empty($program->photo_details->{$image_size}->url) ? $program->photo_details->{$image_size}->url : ''; ?>
            ...
            <?php if (! empty($program->teacher_details->teacher_objects) && ! empty($show_first_teacher_photo)) : ?>
